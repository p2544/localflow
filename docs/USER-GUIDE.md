...
| ระบบ | ไฟล์ที่ใช้ |
|---|---|
| Windows 11 (x64) | `LocalFlow_0.1.4_x64-setup.exe` (NSIS) หรือ `LocalFlow_0.1.4_x64_en-US.msi` |
| macOS Apple Silicon (M1–M4) | `LocalFlow_0.1.4_aarch64.dmg` |
| macOS Intel | build จาก source (ดูข้อ 7 — GitHub เลิกให้บริการ Intel runner แล้ว) |

> เวอร์ชันล่าสุด: **v0.1.4** — https://github.com/p2544/localflow/releases/tag/v0.1.4
...
4. อนุญาตสิทธิ์ 3 อย่างเมื่อระบบถาม (ตัว onboarding ในแอปมีปุ่มขอสิทธิ์/พาไปหน้า Settings ที่ถูกต้องให้เลย):
   - **Microphone** — เพื่ออัดเสียง
   - **Accessibility** — เพื่อพิมพ์ข้อความลงแอปอื่น
   - **Input Monitoring** — เพื่อให้ปุ่มลัดทำงานทุกแอป

   > **v0.1.4:** ตอนเปิดแอปครั้งแรกจะเด้ง prompt ขอ **Accessibility** ให้เลย (หรือกดปุ่ม **Request permission** ในหน้า Permissions) — ติ๊กอนุญาต LocalFlow ในหน้า Privacy & Security → Accessibility แล้ว **ปิด-เปิดแอปใหม่** ถ้ายังไม่ได้อนุญาต แอปจะแจ้ง error ชัดเจนตอนพยายามพิมพ์ (แทนที่จะเงียบเฉยแล้วข้อความไม่ลงเหมือนเวอร์ชันก่อน)
...
2. **Permissions** — (macOS) กดปุ่ม **Request permission** เพื่อขอสิทธิ์ Accessibility แล้วติ๊กอนุญาต LocalFlow (มีปุ่มเปิด System Settings ให้ด้วย)
3. **Download AI models** — กดดาวน์โหลดโมเดล (เก็บในเครื่อง ใช้ครั้งเดียว):
   - **Whisper Large-v3 Turbo Q5** (~547 MB) — แปลงเสียงเป็นข้อความ เร็วและแม่นครอบคลุม 100+ ภาษา (ค่าเริ่มต้น)
   - **Pathumma Thai Large-v3 Q5** (~1.1 GB) — โมเดลเฉพาะภาษาไทยจาก NECTEC แม่นสุดสำหรับไทย (ดาวน์โหลดถ้าเน้นใช้ไทย)
...
| **Models** | ดาวน์โหลด/สลับ/ลบโมเดล — **Large-v3 Turbo** (เร็ว, หลายภาษา, ค่าเริ่มต้น) / **Pathumma Thai** (ไทยแม่นสุด) / **Large-v3** (แม่นสุดแบบทั่วไป, ช้ากว่า Turbo) |
...
| พูดแล้วข้อความไม่แทรกเข้าแอป (macOS) | ส่วนใหญ่คือยังไม่ได้ให้สิทธิ์ **Accessibility** — v0.1.4 แอปจะแจ้ง error บอกให้ไปเปิดใน Privacy & Security → Accessibility → เปิด LocalFlow แล้ว**ปิด-เปิดแอปใหม่** |
| ข้อความไม่ลงช่อง แต่ pill ขึ้น "Copied" | แอปปลายทางบล็อก paste — เปลี่ยน Insert method เป็น **Type** ใน Settings |
| ภาษาไทยถอดผิดเยอะ | Models → ดาวน์โหลด/เปลี่ยนเป็น **Pathumma Thai Large-v3** (เฉพาะไทย) และตั้ง Language = ไทย (อย่าใช้ auto) |
| ช้าตอนพูดครั้งแรกหลังเปิดเครื่อง | ปกติ — โมเดลกำลังโหลดเข้า RAM ครั้งต่อไปจะเร็ว |
| เครื่อง RAM น้อย (<8GB) | ใช้ **Large-v3 Turbo** (เล็กสุดในตัวเลือกตอนนี้ ~547 MB) และ/หรือปิด AI cleanup ใน Settings |
